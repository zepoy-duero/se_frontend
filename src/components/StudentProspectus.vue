          ...
          >
            <template v-slot:top>
              <v-toolbar flat>
                ...
                <v-spacer></v-spacer>
                <v-btn color="primary" @click="createStudentProspectus">Add Student Prospectus</v-btn>
              </v-toolbar>
            </template>
  ...
  export default {
    ...
    methods: {
      fetchStudentProspectuses() {
        apiClient.get('/student-prospectuses')
          .then(response => {
      ...
      editStudentProspectus(studentProspectusId) {
        this.$router.push({ name: 'EditStudentProspectus', params: { id: studentProspectusId } });
      },
      deleteStudentProspectus(studentProspectusId) {
        apiClient.delete(`/student-prospectuses/${studentProspectusId}`)
          .then(() => {
            this.fetchStudentProspectuses();
          })
          .catch(error => {
